...
M.CarouselView = M.View.extend(
    /** @scope M.CarouselView.prototype */ {
            ...
            if(parseInt($('#' + this.id).css('opacity')) > 0 || $('#' + this.id).parent().width() === 0 || $('#' + this.id).parent().width() === this.lastWidth) {
                ...
                window.setTimeout(function() {
                    ...
                    if(that.sizeCalculator === M.CAROUSEL_SIZE_CONTENT) {
                        $('#' + that.id + ' ul.tmp-carousel-list li.tmp-carousel-item').each(function() {
                    ...
                    if(that.iScroll) {
                        ...
                        that.iScroll.scrollToElement(' li.tmp-carousel-item:nth-child(' + (that.activeItem > 1 ? that.activeItem : 1) + ')', 100);
                    } else {
                        that.iScroll = new iScroll(that.id, {
                            ...
                            onScrollEnd: function () {
                                ...
                                if(that.direction === M.HORIZONTAL) {
                                    var width = parseInt($('#' + that.id + ' ul.tmp-carousel-list li.tmp-carousel-item').css('width'));
                                    nextItem = Math.abs(Math.floor(that.iScroll.x / width)) + 1;
                                } else {
                                    var height = parseInt($('#' + that.id + ' ul.tmp-carousel-list li.tmp-carousel-item').css('height'));
         ...
         */
        next: function() {
            var nextItem = $('#' + this.id + ' .tmp-carousel-list li.tmp-carousel-item:nth-child(' + this.activeItem + ')').next();
         ...
         */
        prev: function() {
            var prevItem = $('#' + this.id + ' .tmp-carousel-list li.tmp-carousel-item:nth-child(' + this.activeItem + ')').prev();
